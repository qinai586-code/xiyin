...
SECTION_CHOICES = ["all", "syntax", "security_check", "g4", "l3_l4", "c4", "persona", "audit", "governance"]
...
def section_syntax():
    def ast_parse(_stack):
        ...
        json.loads(_repo_path("L0_RUNTIME", "L2_CENTRAL", "C7_PLUGINS", "manifest.json").read_text(encoding="utf-8"))
...
def _select_sections(section):
    ...
    ordered = ["syntax", "security_check", "g4", "l3_l4", "c4", "persona", "audit", "governance"] if section == "all" else [section]
    results = []
    for name in ordered:
