...
def _require_admin_user() -> None:
    ...
    if current_user.casefold() != EXPECTED_USER.casefold():
        raise PermissionError("G4 executor must be run by SJ_Admin")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.self_test:
            _run_self_test()
            _print_json({"ok": True, "self_test": "G4_SELF_TEST_OK", "dry_run": True, "writes_performed": False})
            return 0

        event = _parse_event_json(args.event_json)
        _print_json(evaluate_event(event))
        return 0
